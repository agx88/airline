...
describe('Starting tests for airline-angular-test', function() {
  ...
  it('website title should be airline-angular-test', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('airline-angular-test');
    })
  });

  it('network-name should be airline@0.0.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('airline@0.0.0.bna');
    });
  });

  it('navbar-brand should be airline-angular-test',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('airline-angular-test');
    });
  });
    ...
    it('Flight table should have 6 columns',() => {
      page.navigateTo('/Flight');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('UpdateAircraftInBatch component should be loadable',() => {
      page.navigateTo('/UpdateAircraftInBatch');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('UpdateAircraftInBatch');
      });
    });
